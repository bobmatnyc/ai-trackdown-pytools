...
from ai_trackdown_pytools.utils.frontmatter import FrontmatterParser, StatusWorkflowValidator
from ai_trackdown_pytools.utils.git import GitUtils
from ai_trackdown_pytools.utils.templates import TemplateManager
from ai_trackdown_pytools.utils.validation import SchemaValidator, validate_relationships
...
class TestValidationRegressions:
    ...
    def test_circular_dependency_false_positive_bug(self):
        ...
        tickets = [
            ...
            {"id": "TSK-0004", "dependencies": ["TSK-0001"]},  # Depends on TSK-0001, not circular
            {"id": "TSK-0005", "dependencies": ["TSK-0002", "TSK-0003"]},  # Multiple deps, not circular
        ]
        ...
        assert len(circular_errors) == 0, f"False positive circular dependency: {circular_errors}"
    ...
    def test_id_pattern_validation_edge_cases_bug(self):
        ...
        for ticket_type, ticket_id in edge_case_ids:
            ...
            assert re.match(pattern, ticket_id), f"Valid ID {ticket_id} failed pattern {pattern}"
    ...
    def test_status_transition_validation_bug(self):
        ...
        for ticket_type, from_status, to_status in valid_transitions:
            ...
            assert result.valid, f"Valid transition {from_status} -> {to_status} for {ticket_type} was rejected"
...
"""
        ...
        assert "<b>Safe HTML</b>" in result  # raw_html should be unescaped (marked safe)
    ...
    def test_template_infinite_recursion_bug(self):
        ...
        for template_name in templates:
            try:
                ...
                assert "recursion" in error_msg or "loop" in error_msg or "depth" in error_msg
...
""")
        ...
        with patch.object(template_manager, '_template_dirs', [temp_dir / "templates"]):
            ...
            for traversal_name in traversal_attempts:
                ...
                assert result is None, f"Directory traversal succeeded: {traversal_name}"
...
pytest.mark.regression = pytest.mark.regression if hasattr(pytest.mark, 'regression') else pytest.mark.skipif(False, reason="Regression tests")
