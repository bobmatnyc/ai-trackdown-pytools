...
from ai_trackdown_pytools.utils.validation import (
    ...
    validate_project_structure, validate_task_file,
    validate_ticket_file, validate_id_format,
    validate_relationships, validate_task_data,
    validate_epic_data, validate_issue_data,
...
class TestProjectStructureValidation:
    ...
    def test_validate_project_structure_valid(self, mock_is_dir, mock_exists):
        ...
        def exists_side_effect(path):
            ...
            return any(part in path_str for part in [
                ".aitrackdown", "config.yaml", "tasks", "issues", "epics", "prs"
            ])
    ...
    def test_validate_project_structure_missing_dirs(self, mock_exists):
        ...
        mock_exists.side_effect = lambda p: ".aitrackdown" in str(p) and "config" not in str(p)
...
# Test Epic"""
        ...
        with patch('builtins.open', mock_open(read_data=epic_content)):
            with patch('pathlib.Path.exists', return_value=True):
                result = validate_ticket_file(Path("/epics/EP-001.md"), ticket_type="epic")
...
class TestRelationshipValidation:
    ...
    def test_validate_relationships_invalid_reference(self):
        ...
        tickets = [
            {
                "id": "TSK-001",
                "dependencies": ["TSK-999"]  # Non-existent
            }
        ]
