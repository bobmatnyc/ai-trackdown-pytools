...
class TestVersionConstants:
    ...
    def test_features_dict(self):
        ...
        for feature, version in FEATURES.items():
            assert validate_version_string(version), f"Invalid version for {feature}: {version}"
...
class TestVersionParametrized:
    ...
    @pytest.mark.parametrize("version_str,is_dev", [
        ("0.1.0", True),
        ("0.9.0", True),
        ("1.0.0", False),
        ("2.1.3", False),
    ...
    @pytest.mark.parametrize("version_str,has_pre", [
        ("1.0.0", False),
        ("1.0.0-alpha", True),
        ("1.0.0-beta.1", True),
        ("1.0.0-rc.1", True),
