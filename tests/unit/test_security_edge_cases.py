            ...
            try:
                ...
                try:
                    ...
                    if config_path.exists():
                        assert str(config_path.resolve()).startswith(str(temp_dir.resolve()))
...
class TestPrivilegeEscalationPrevention:
    ...
    def test_file_permission_safety(self, temp_dir: Path):
        ...
        if os.name != 'nt':  # Unix-like systems
            assert not (file_mode & 0o020) or (file_mode & 0o040)  # Group write only if group read
...
pytest.mark.security = pytest.mark.skipif(
    os.environ.get("SKIP_SECURITY_TESTS") == "1",
    reason="Security tests skipped"
)