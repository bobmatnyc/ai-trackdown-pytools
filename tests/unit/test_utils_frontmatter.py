...
from ai_trackdown_pytools.utils.frontmatter import (
    FrontmatterParser, StatusWorkflowValidator, ValidationResult,
    parse_ticket_file, validate_ticket_data, write_ticket_file,
    FrontmatterError
)
...
""")
    ...
    def test_write_file_success(self):
        ...
        try:
            result = self.parser.write_file(temp_path, frontmatter_data, markdown_content)
    ...
    def test_roundtrip_parse_write(self):
        ...
        try:
            # Write original data
            write_result = self.parser.write_file(temp_path, original_frontmatter, original_markdown)
            assert write_result.valid
            
            # Parse it back
            parsed_frontmatter, parsed_markdown, parse_result = self.parser.parse_file(temp_path)
...
"""
        ...
        frontmatter, markdown, result = self.parser_with_validation.parse_string(valid_content)
...
"""
        ...
        frontmatter, markdown, result = self.parser_with_validation.parse_string(invalid_content)
...
class TestStatusWorkflowValidator:
    ...
    def test_valid_task_status_transitions(self):
        ...
        # Valid transitions
        result = self.validator.validate_status_transition("task", "open", "in_progress")
        assert result.valid
        
        result = self.validator.validate_status_transition("task", "in_progress", "completed")
        assert result.valid
        
        result = self.validator.validate_status_transition("task", "open", "cancelled")
        assert result.valid
        
        result = self.validator.validate_status_transition("task", "blocked", "in_progress")
        assert result.valid
    ...
    def test_invalid_task_status_transitions(self):
        ...
        result = self.validator.validate_status_transition("task", "cancelled", "in_progress")
        assert not result.valid
        
        result = self.validator.validate_status_transition("task", "completed", "cancelled")
        assert not result.valid
    
    def test_valid_epic_status_transitions(self):
        """Test valid epic status transitions."""
        result = self.validator.validate_status_transition("epic", "planning", "in_progress")
        assert result.valid
        
        result = self.validator.validate_status_transition("epic", "in_progress", "completed")
        assert result.valid
        
        result = self.validator.validate_status_transition("epic", "planning", "on_hold")
        assert result.valid
    
    def test_valid_issue_status_transitions(self):
        """Test valid issue status transitions."""
        result = self.validator.validate_status_transition("issue", "open", "in_progress")
        assert result.valid
        
        result = self.validator.validate_status_transition("issue", "in_progress", "resolved")
        assert result.valid
        
        result = self.validator.validate_status_transition("issue", "resolved", "verified")
        assert result.valid
        
        result = self.validator.validate_status_transition("issue", "verified", "closed")
        assert result.valid
    ...
    def test_get_valid_transitions(self):
        ...
        completed_transitions = self.validator.get_valid_transitions("task", "completed")
    ...
    def test_invalid_current_status(self):
        ...
        result = self.validator.validate_status_transition("task", "invalid_status", "open")
        assert not result.valid
    ...
    def test_invalid_new_status(self):
        ...
        result = self.validator.validate_status_transition("task", "open", "invalid_status")
        assert not result.valid
