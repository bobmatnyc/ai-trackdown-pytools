...
from ai_trackdown_pytools.cli import (
    app, main, version_callback, config, doctor, edit, search, validate,
    info, health
)
...
class TestCLICommands:
    ...
    def test_doctor_command(self, mock_git_utils, mock_config_class, mock_project, 
                          mock_project_health, mock_check_health, runner):
...
class TestEditCommand:
    ...
    def test_edit_task_success(self, mock_editor, mock_task_manager_class, 
                             mock_project, runner):
        ...
        mock_editor.open_file.assert_called_once_with(Path("/project/tasks/TSK-0001.md"), None)
...
class TestSearchCommand:
    ...
    def test_search_with_filters(self, mock_task_manager_class, mock_project, runner):
        ...
        result = runner.invoke(app, ["search", "bug", "--status", "open", "--type", "bug"])
...
class TestValidateCommand:
    ...
    def test_validate_project_success(self, mock_validate_structure, mock_project, runner):
    ...
    @patch('ai_trackdown_pytools.core.project.Project')
    @patch('ai_trackdown_pytools.utils.validation.validate_project_structure')
    def test_validate_project_failure(self, mock_validate_structure, mock_project, runner):
    ...
    def test_validate_tasks(self, mock_validate_task, mock_task_manager_class, 
                          mock_project, runner):
        ...
        mock_validate_task.side_effect = [
            ...
            {'valid': False, 'errors': ['Invalid schema'], 'warnings': ['Missing description']}
        ]
...
class TestSubcommandRegistration:
    ...
    def test_all_subcommands_registered(self):
        ...
        expected_commands = [
            'init', 'status', 'create', 'template', 'validate',
            'task', 'issue', 'epic', 'pr', 'search', 'portfolio',
            'sync', 'ai', 'migrate', 'info', 'health', 'config',
            'doctor', 'version', 'edit'
        ]
    ...
    def test_subcommand_help_text(self):
        ...
        assert app.registered_groups['init'].help == "Initialize AI Trackdown project structure"
        assert app.registered_groups['status'].help == "Show project and task status"
        assert app.registered_groups['create'].help == "Create new tasks, projects, or issues"
        assert app.registered_groups['template'].help == "Manage and apply templates"
        assert app.registered_groups['validate'].help == "Validate tickets, schemas, and relationships"
