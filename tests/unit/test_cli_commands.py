...
from ai_trackdown_pytools.commands import (
    init, create, status, template, validate_typer,
    task, epic, issue, pr, search
)
...
class TestMainCLI:
    ...
    def test_app_project_dir_nonexistent(self):
        ...
        result = self.runner.invoke(app, ["--project-dir", "/nonexistent/path", "--help"])
...
class TestInitCommand:
    ...
    def test_init_project_success(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert result.exit_code == 0
            assert "initialized successfully" in result.stdout.lower() or "created" in result.stdout.lower()
    ...
    def test_init_project_custom_name(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('os.getcwd', return_value=str(project_path)):
                result = self.runner.invoke(app, ["init", "project", "--name", "Custom Project"])
...
class TestCLIIntegration:
    ...
    def test_full_workflow(self):
        ...
        with patch('os.getcwd', return_value=str(project_path)):
            # Initialize project
            result = self.runner.invoke(app, ["init", "project", "--name", "Workflow Test"])
            assert result.exit_code == 0
            
            # Create a task
            result = self.runner.invoke(app, [
                "create", "task", "Workflow Task",
                "--priority", "high",
                "--tag", "test"
            ])
            assert result.exit_code == 0
