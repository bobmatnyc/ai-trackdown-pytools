...
class TestJSONOutputFormat:
    ...
    def test_search_json_output(self, mock_task_mgr, mock_exists, output_runner):
        ...
        # Mock task data
        mock_tasks = [
            Mock(id="TSK-001", title="Task 1", description="Description 1", 
                 tags=["test"], status="open", priority="high", assignees=["alice"],
                 created_at="2024-01-01T10:00:00", updated_at="2024-01-01T10:00:00"),
            Mock(id="TSK-002", title="Task 2", description="Description 2", 
                 tags=["feature"], status="completed", priority="medium", assignees=["bob"],
                 created_at="2024-01-02T10:00:00", updated_at="2024-01-02T15:00:00")
        ]
...
class TestCSVOutputFormat:
    ...
    def test_search_csv_output(self, mock_task_mgr, mock_exists, output_runner):
        ...
        mock_tasks = [
            Mock(id="TSK-001", title="Task 1", description="First task", 
                 tags=["test", "urgent"], status="open", priority="high", assignees=["alice"]),
            Mock(id="TSK-002", title="Task with, comma", description="Second task", 
                 tags=["feature"], status="in_progress", priority="medium", assignees=["bob", "charlie"]),
            Mock(id="TSK-003", title="Task with \"quotes\"", description="Third task", 
                 tags=[], status="completed", priority="low", assignees=[])
        ]
        mock_task_mgr.return_value.list_tasks.return_value = mock_tasks
    ...
    def test_status_csv_export(self, mock_task_mgr, mock_exists, output_runner):
        ...
        mock_tasks = [
            ...
            Mock(id="TSK-002", status="completed", priority="medium", created_at="2024-01-02")
        ]
        mock_task_mgr.return_value.list_tasks.return_value = mock_tasks
...
class TestXMLOutputFormat:
    ...
    def test_search_xml_output(self, mock_task_mgr, mock_exists, output_runner):
        ...
        mock_tasks = [
            Mock(id="TSK-001", title="XML Test Task", description="Task for XML testing",
                 tags=["xml", "test"], status="open", priority="medium")
        ]
        mock_task_mgr.return_value.list_tasks.return_value = mock_tasks
        ...
        if result.output.strip().startswith('<?xml') or result.output.strip().startswith('<'):
...
class TestYAMLOutputFormat:
    ...
    def test_search_yaml_output(self, mock_task_mgr, mock_exists, output_runner):
        ...
        mock_tasks = [
            Mock(id="TSK-001", title="YAML Test", description="Testing YAML output",
                 tags=["yaml"], status="open", priority="low")
        ]
        mock_task_mgr.return_value.list_tasks.return_value = mock_tasks
...
class TestTableFormattingOptions:
    ...
    def test_table_styles(self, mock_task_mgr, mock_exists, output_runner):
        ...
        mock_tasks = [
            ...
            Mock(id="TSK-002", title="Style Test 2", status="completed", priority="low")
        ]
        mock_task_mgr.return_value.list_tasks.return_value = mock_tasks
        ...
        for style in table_styles:
            result = output_runner.invoke(app, ["search", "style", "--table-style", style])
            assert result.exit_code == 0
    ...
    def test_column_customization(self, mock_task_mgr, mock_exists, output_runner):
        ...
        mock_tasks = [
            Mock(id="TSK-001", title="Column Test", status="open", priority="medium",
                 assignees=["alice"], created_at="2024-01-01", tags=["test"])
        ]
        mock_task_mgr.return_value.list_tasks.return_value = mock_tasks
        
        # Test custom column selection
        result = output_runner.invoke(app, [
            "search", "column", "--columns", "id,title,status,priority"
        ])
        assert result.exit_code == 0
    ...
    def test_pagination_options(self, mock_task_mgr, mock_exists, output_runner):
        ...
        # Mock large dataset
        mock_tasks = [
            Mock(id=f"TSK-{i:03d}", title=f"Pagination Test {i}", status="open", priority="medium")
            for i in range(1, 51)
        ]
        mock_task_mgr.return_value.list_tasks.return_value = mock_tasks
        ...
        # Test pagination
        result = output_runner.invoke(app, [
            "search", "pagination", "--page-size", "10", "--page", "1"
        ])
        assert result.exit_code == 0
    ...
    def test_sorting_options(self, mock_task_mgr, mock_exists, output_runner):
        ...
        mock_tasks = [
            Mock(id="TSK-003", title="C Task", status="open", priority="low", created_at="2024-01-03"),
            Mock(id="TSK-001", title="A Task", status="completed", priority="high", created_at="2024-01-01"),
            Mock(id="TSK-002", title="B Task", status="in_progress", priority="medium", created_at="2024-01-02")
        ]
        mock_task_mgr.return_value.list_tasks.return_value = mock_tasks
        ...
        for field, order in sort_options:
            result = output_runner.invoke(app, [
                "search", "sort", "--sort-by", field, "--sort-order", order
            ])
            assert result.exit_code == 0
...
class TestErrorConditions:
    ...
    def test_invalid_project_directory(self, output_runner):
        ...
        result = output_runner.invoke(app, ["--project-dir", "/nonexistent/path", "info"])
    ...
    def test_file_not_found_errors(self, output_runner):
        ...
        result = output_runner.invoke(app, ["--config", "/nonexistent/config.yaml", "info"])
...
class TestValidationErrors:
    ...
    def test_invalid_priority_values(self, mock_create, mock_exists, output_runner):
        ...
        result = output_runner.invoke(app, [
            "task", "create", "Invalid Priority Task", "--priority", "invalid"
        ])
    ...
    def test_invalid_status_values(self, mock_create, mock_exists, output_runner):
        ...
        result = output_runner.invoke(app, [
            "task", "update", "TSK-001", "--status", "invalid_status"
        ])
    ...
    @patch('ai_trackdown_pytools.core.project.Project.exists')
    @patch('ai_trackdown_pytools.core.task.TaskManager')
    def test_circular_dependency_detection(self, mock_task_mgr, mock_exists, output_runner):
        """Test detection of circular dependencies."""
        mock_exists.return_value = True
        
        # Mock creating a task that would create circular dependency
        mock_task_mgr.return_value.create_task.side_effect = ValueError("Circular dependency detected")
        
        result = output_runner.invoke(app, [
            "task", "create", "Circular Task", "--parent", "TSK-001", "--dependency", "TSK-001"
        ])
...
class TestOutputRedirection:
    ...
    def test_output_to_file(self, mock_task_mgr, mock_exists, output_runner):
        ...
        mock_tasks = [
            Mock(id="TSK-001", title="File Output Test", status="open", priority="medium")
        ]
        mock_task_mgr.return_value.list_tasks.return_value = mock_tasks
...
class TestUnicodeHandling:
    ...
    def test_unicode_search_queries(self, mock_task_mgr, mock_exists, output_runner):
        ...
        mock_tasks = [
            Mock(id="TSK-001", title="测试任务", description="Chinese test task", 
                 tags=["测试"], status="open"),
            Mock(id="TSK-002", title="Тестовая задача", description="Russian test task", 
                 tags=["тест"], status="open")
        ]
        mock_task_mgr.return_value.list_tasks.return_value = mock_tasks
    ...
    @patch('ai_trackdown_pytools.core.project.Project.exists')
    @patch('ai_trackdown_pytools.core.task.TaskManager.create_task')
    def test_special_characters_in_descriptions(self, mock_create, mock_exists, output_runner):
        ...
        result = output_runner.invoke(app, [
            "task", "create", "Special Chars Test",
...
class TestLargeDataHandling:
    ...
    def test_large_search_results(self, mock_task_mgr, mock_exists, output_runner):
        ...
        mock_tasks = [
            Mock(id=f"TSK-{i:05d}", title=f"Large Dataset Task {i}", 
                 description=f"Task {i} in large dataset test",
                 tags=[f"tag{i%10}"], status=["open", "in_progress", "completed"][i%3],
    ...
    def test_many_tags_and_assignees(self, mock_create, mock_exists, output_runner):
        ...
        result = output_runner.invoke(app, [
            "task", "create", "Many Tags Test",
            "--tag"] + many_tags[:10] + [  # Limit to avoid command line length issues
            "--assignee"] + many_assignees[:5]
        )
        assert result.exit_code == 0
