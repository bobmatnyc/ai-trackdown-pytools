...
class TestMainCLIOptions:
    ...
    def test_version_option_short(self, cli_runner):
        ...
        assert "AI Trackdown PyTools" in result.output
    ...
    def test_project_dir_option_long(self, cli_runner, temp_project_dir):
        ...
        result = cli_runner.invoke(app, ["--project-dir", str(temp_project_dir), "info"])
        assert result.exit_code == 0
...
class TestCommandGroups:
    ...
    def test_pr_command_help(self, cli_runner):
        ...
        assert result.exit_code == 0
        assert "Pull request management" in result.output
...
class TestInitCommandOptions:
    ...
    def test_init_project_with_template(self, cli_runner):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            project_path = Path(temp_dir) / "new_project"
            result = cli_runner.invoke(app, ["init", "project", str(project_path), "--template", "default"])
            assert result.exit_code == 0
    
    def test_init_project_force_option(self, cli_runner, temp_project_dir):
        """Test project initialization with --force option."""
        result = cli_runner.invoke(app, ["init", "project", str(temp_project_dir), "--force"])
        assert result.exit_code == 0
        assert "Project initialized successfully" in result.output
    ...
    def test_init_project_no_git(self, cli_runner):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            project_path = Path(temp_dir) / "new_project"
            result = cli_runner.invoke(app, ["init", "project", str(project_path), "--no-git"])
            assert result.exit_code == 0
...
class TestCreateCommandOptions:
    ...
    def test_create_task_with_priority(self, mock_create, mock_exists, cli_runner):
        ...
        for priority in ["low", "medium", "high", "critical"]:
            result = cli_runner.invoke(app, [
                "create", "task", f"Test Task {priority}",
                "--priority", priority
            ])
            assert result.exit_code == 0
...
class TestTaskCommandOptions:
    ...
    def test_task_create_edit_flag(self, mock_editor, mock_create, mock_exists, cli_runner):
...
class TestStatusCommandOptions:
    ...
    def test_status_project_basic(self, mock_git, mock_task_mgr, mock_load, mock_exists, cli_runner):
    ...
    @patch('ai_trackdown_pytools.core.task.TaskManager')
    @patch('ai_trackdown_pytools.utils.git.GitUtils')
    def test_status_project_verbose(self, mock_git, mock_task_mgr, mock_load, mock_exists, cli_runner):
    ...
    @patch('ai_trackdown_pytools.core.task.TaskManager')
    @patch('ai_trackdown_pytools.utils.git.GitUtils')
    def test_status_project_custom_path(self, mock_git, mock_task_mgr, mock_load, mock_exists, cli_runner, temp_project_dir):
...
class TestSearchCommandOptions:
    ...
    def test_search_basic_query(self, mock_task_mgr, mock_exists, cli_runner):
        ...
        mock_exists.return_value = True
        mock_tasks = [
            Mock(id="TSK-001", title="Test Task", description="Test description", tags=["test"], status="open"),
            Mock(id="TSK-002", title="Another Task", description="Another description", tags=["feature"], status="in_progress")
        ]
    ...
    def test_search_with_type_filter(self, mock_task_mgr, mock_exists, cli_runner):
        ...
        mock_exists.return_value = True
        mock_tasks = [
            Mock(id="TSK-001", title="Test Task", description="Test description", tags=["test", "task"], status="open"),
        ]
        mock_task_mgr.return_value.list_tasks.return_value = mock_tasks
    ...
    def test_search_with_limit(self, mock_task_mgr, mock_exists, cli_runner):
        ...
        mock_exists.return_value = True
        mock_tasks = [Mock(id=f"TSK-{i:03d}", title=f"Task {i}", description="Test", tags=["test"], status="open") for i in range(20)]
        mock_task_mgr.return_value.list_tasks.return_value = mock_tasks
...
class TestEditCommandOptions:
    ...
    @patch('ai_trackdown_pytools.core.task.TaskManager')
    @patch('ai_trackdown_pytools.utils.editor.EditorUtils.open_file')
    def test_edit_task_with_editor(self, mock_editor, mock_task_mgr, mock_exists, cli_runner):
...
class TestValidateCommandOptions:
    ...
    def test_validate_tasks(self, mock_validate_task, mock_task_mgr, mock_exists, cli_runner):
    ...
    @patch('ai_trackdown_pytools.core.project.Project.exists')
    @patch('ai_trackdown_pytools.utils.validation.validate_project_structure')
    def test_validate_with_custom_path(self, mock_validate, mock_exists, cli_runner, temp_project_dir):
        """Test validation with custom path."""
        mock_exists.return_value = True
        mock_validate.return_value = {"valid": True, "errors": [], "warnings": []}
        
        result = cli_runner.invoke(app, ["validate", "project", "--path", str(temp_project_dir)])
        assert result.exit_code == 0
...
class TestBuiltinCommands:
    ...
    def test_health_command_pass(self, mock_health, cli_runner):
        ...
        mock_health.return_value = {
            "overall": True,
            "checks": {
                "dependencies": {"status": True, "message": "All dependencies available"}
            }
        }
    ...
    @patch('ai_trackdown_pytools.core.config.Config.load')
    @patch('ai_trackdown_pytools.utils.git.GitUtils')
    def test_doctor_command(self, mock_git, mock_config, mock_exists, mock_project_health, mock_health, cli_runner):
        """Test doctor command."""
        mock_health.return_value = {
            "overall": True,
        ...
        mock_config.return_value = Mock(config_path="/test/config", project_root="/test")
...
class TestInteractivePrompts:
    ...
    def test_interactive_task_creation(self, mock_prompt, mock_create, mock_exists, cli_runner):
    ...
    @patch('ai_trackdown_pytools.core.task.TaskManager.create_task')
    @patch('rich.prompt.Prompt.ask')
    def test_interactive_task_creation_no_title(self, mock_prompt, mock_create, mock_exists, cli_runner):
...
class TestErrorConditions:
    ...
    def test_invalid_config_file(self, cli_runner):
        ...
        result = cli_runner.invoke(app, ["--config", "/nonexistent/config.yaml", "info"])
...
class TestOutputFormats:
    ...
    def test_table_output_format(self, mock_task_mgr, mock_exists, cli_runner):
        ...
        mock_exists.return_value = True
        mock_tasks = [
            Mock(id="TSK-001", title="Task 1", description="Desc 1", tags=["test"], status="open"),
            Mock(id="TSK-002", title="Task 2", description="Desc 2", tags=["feature"], status="done")
        ]
        mock_task_mgr.return_value.list_tasks.return_value = mock_tasks
...
class TestCLIWorkflows:
    ...
    def test_create_then_search_workflow(self, mock_task_mgr, mock_exists, cli_runner):
        ...
        # Mock task creation
        created_task = Mock(id="TSK-001", title="Created Task", description="Test", tags=["test"], status="open")
...
class TestGlobalOptionCombinations:
    ...
    def test_verbose_with_config_file(self, mock_system_info, cli_runner):
        ...
        try:
            result = cli_runner.invoke(app, ["--verbose", "--config", config_path, "info"])
            assert result.exit_code == 0
        finally:
            os.unlink(config_path)
    ...
    def test_verbose_with_project_dir(self, cli_runner, temp_project_dir):
        ...
        with patch('ai_trackdown_pytools.utils.system.get_system_info') as mock_info:
            ...
            result = cli_runner.invoke(app, ["--verbose", "--project-dir", str(temp_project_dir), "info"])
            assert result.exit_code == 0
