...
class TestCompleteTicketLifecycle:
    ...
    def test_complete_project_epic_issue_task_workflow(self, e2e_runner, e2e_project_setup):
        ...
        try:
            ...
            with patch('ai_trackdown_pytools.core.task.TaskManager.create_task') as mock_create_epic:
            ...
            with patch('ai_trackdown_pytools.core.task.TaskManager.create_task') as mock_create_issue:
                mock_issue = Mock(id="ISS-001", title="E2E Test Issue", type="issue")
                mock_create_issue.return_value = mock_issue
                
                result = e2e_runner.invoke(app, [
                    "issue", "create", "E2E Test Issue",
            ...
            for i, title in enumerate(task_titles, 1):
                with patch('ai_trackdown_pytools.core.task.TaskManager.create_task') as mock_create_task:
                    mock_task = Mock(id=f"TSK-{i:03d}", title=title, type="task")
                    mock_create_task.return_value = mock_task
                    
                    result = e2e_runner.invoke(app, [
                        "task", "create", title,
                        "--description", f"Task {i} for E2E testing",
                        "--issue", "ISS-001",
                    ...
                    assert result.exit_code == 0, f"Task {i} creation failed: {result.output}"
                    created_items["tasks"].append(f"TSK-{i:03d}")
            
            # Step 5: Create a subtask
            with patch('ai_trackdown_pytools.core.task.TaskManager.create_task') as mock_create_subtask:
                mock_subtask = Mock(id="TSK-005", title="Subtask for testing", type="task")
                mock_create_subtask.return_value = mock_subtask
                
                result = e2e_runner.invoke(app, [
                    "task", "create", "Setup Test Data",
                    "--description", "Subtask for test data preparation",
                    "--parent", "TSK-001",
                    "--assignee", "tester1",
                    "--priority", "medium",
                    "--tag", "subtask"
                ])
                assert result.exit_code == 0, f"Subtask creation failed: {result.output}"
            ...
            for task_id, status in status_updates:
                with patch('ai_trackdown_pytools.core.task.TaskManager.update_task') as mock_update:
                    result = e2e_runner.invoke(app, [
                        "task", "update", task_id,
                        "--status", status
                    ])
            ...
            for task_id, comment in comment_data:
                with patch('ai_trackdown_pytools.core.task.TaskManager.add_comment') as mock_comment:
                    result = e2e_runner.invoke(app, [
            ...
            with patch('ai_trackdown_pytools.core.project.Project.exists') as mock_exists, \
                 patch('ai_trackdown_pytools.core.task.TaskManager') as mock_task_mgr:
                
                mock_exists.return_value = True
                
                # Mock search results
                mock_tasks = [
                    Mock(id="TSK-001", title="Implement E2E Test Framework", 
                         description="Task 1 for E2E testing", tags=["e2e", "testing"], status="in_progress"),
                    Mock(id="TSK-002", title="Write Test Cases", 
                         description="Task 2 for E2E testing", tags=["e2e", "testing"], status="completed"),
                    Mock(id="EP-001", title="E2E Test Epic", 
                         description="Epic for end-to-end testing", tags=["epic"], status="active"),
                    Mock(id="ISS-001", title="E2E Test Issue", 
                         description="Issue for testing epic association", tags=["issue"], status="open")
                ]
                ...
                # Search by type
                result = e2e_runner.invoke(app, ["search", "test", "--type", "task", "--limit", "10"])
                assert result.exit_code == 0, f"Type search failed: {result.output}"
                
                # Search by status
                result = e2e_runner.invoke(app, ["search", "E2E", "--status", "in_progress"])
                assert result.exit_code == 0, f"Status search failed: {result.output}"
            
            # Step 9: Generate reports and status
            with patch('ai_trackdown_pytools.core.project.Project.exists') as mock_exists, \
                 patch('ai_trackdown_pytools.core.project.Project.load') as mock_load, \
                 patch('ai_trackdown_pytools.core.task.TaskManager') as mock_task_mgr, \
                 patch('ai_trackdown_pytools.utils.git.GitUtils') as mock_git:
                
                mock_exists.return_value = True
                mock_project = Mock(name="E2E Test Project", description="End-to-end testing project")
            ...
            with patch('ai_trackdown_pytools.core.project.Project.exists') as mock_exists, \
                 patch('ai_trackdown_pytools.utils.validation.validate_project_structure') as mock_validate:
                
                mock_exists.return_value = True
                mock_validate.return_value = {"valid": True, "errors": [], "warnings": []}
                
                result = e2e_runner.invoke(app, ["validate", "project"])
                assert result.exit_code == 0, f"Project validation failed: {result.output}"
    ...
    def test_portfolio_backlog_workflow(self, e2e_runner, e2e_project_setup):
        ...
        try:
            ...
            for i, (name, desc) in enumerate(project_data, 1):
                with patch('ai_trackdown_pytools.core.project.Project.create') as mock_create:
                    mock_project = Mock(id=f"PROJ-{i:03d}", name=name)
                    mock_create.return_value = mock_project
                    
                    result = e2e_runner.invoke(app, [
                        "portfolio", "create-project", name,
                        "--description", desc,
            ...
            for i, (title, project, priority) in enumerate(epic_data, 1):
                with patch('ai_trackdown_pytools.core.task.TaskManager.create_task') as mock_create:
                    ...
                    result = e2e_runner.invoke(app, [
                        ...
                        "--project", project,
                        "--priority", priority,
                        "--business-value", f"Value proposition {i}",
                        "--target-date", "2024-12-31"
                    ])
                    created_items["epics"].append(f"EP-{i:03d}")
            
            # Step 3: Create backlog items
            with patch('ai_trackdown_pytools.core.task.TaskManager.list_tasks') as mock_list:
                mock_backlog_items = [
                    Mock(id=f"TSK-{i:03d}", title=f"Backlog Item {i}", 
                         priority=["low", "medium", "high"][i % 3], status="backlog")
                    for i in range(1, 11)
                ]
                mock_list.return_value = mock_backlog_items
                
                result = e2e_runner.invoke(app, ["portfolio", "backlog", "--limit", "10"])
    ...
    def test_sync_and_migration_workflow(self, e2e_runner, e2e_project_setup):
        ...
        try:
            ...
            with patch('ai_trackdown_pytools.utils.git.GitUtils.is_git_repo') as mock_git:
                mock_git.return_value = True
                
                result = e2e_runner.invoke(app, [
                    "sync", "configure", "github",
                    "--token", "test_token",
            ...
            with patch('requests.post') as mock_post:
                mock_post.return_value = Mock(status_code=201, json=lambda: {"number": 43})
    ...
    def test_template_and_automation_workflow(self, e2e_runner, e2e_project_setup):
        ...
        try:
            ...
            with patch('ai_trackdown_pytools.utils.templates.TemplateManager.list_templates') as mock_list:
                mock_list.return_value = ["default", "bug_report", "feature_request", "epic"]
...
"""
            ...
            with patch('ai_trackdown_pytools.core.task.TaskManager.create_task') as mock_create:
                ...
                result = e2e_runner.invoke(app, [
                    "task", "create", "Template Generated Task",
                    "--template", "e2e_test",
                    "--test-type", "e2e"
                ])
            ...
            with patch('ai_trackdown_pytools.utils.validation.SchemaValidator.validate_template') as mock_validate:
                mock_validate.return_value = {"valid": True, "errors": [], "warnings": []}
                
                result = e2e_runner.invoke(app, [
                    "template", "validate", "e2e_test"
                ])
                assert result.exit_code == 0
            
            # Step 5: Export template
            result = e2e_runner.invoke(app, [
                "template", "export", "e2e_test",
                "--output", "/tmp/e2e_test_template.yaml"
            ])
    ...
    def test_comprehensive_validation_workflow(self, e2e_runner, e2e_project_setup):
        ...
        try:
            ...
            with patch('ai_trackdown_pytools.core.project.Project.exists') as mock_exists, \
                 patch('ai_trackdown_pytools.core.task.TaskManager') as mock_task_mgr, \
                 patch('ai_trackdown_pytools.utils.validation.validate_task_file') as mock_validate_task:
            ...
            with patch('ai_trackdown_pytools.core.config.Config.load') as mock_load, \
                 patch('ai_trackdown_pytools.utils.validation.SchemaValidator') as mock_validator_class:
                ...
                mock_validator = Mock()
                mock_validator.validate_config.return_value = {"valid": True, "errors": [], "warnings": []}
            ...
            with patch('ai_trackdown_pytools.utils.health.check_health') as mock_health, \
                 patch('ai_trackdown_pytools.utils.health.check_project_health') as mock_project_health, \
                 patch('ai_trackdown_pytools.core.project.Project.exists') as mock_exists, \
                 patch('ai_trackdown_pytools.core.config.Config.load') as mock_config, \
                 patch('ai_trackdown_pytools.utils.git.GitUtils') as mock_git:
                ...
                mock_config.return_value = Mock(config_path="/test", project_root="/test")
    ...
    def _cleanup_created_items(self, runner: CliRunner, created_items: Dict[str, List[str]]):
        """Clean up all created items in reverse dependency order."""
        
        # Delete in reverse order: tasks → issues → epics → projects
        cleanup_order = ["tasks", "issues", "epics", "projects", "templates", "ai_sessions", "ai_tasks", 
                        "synced_items", "migrated_items", "automated_tasks"]
            ...
            if item_type in created_items:
                for item_id in created_items[item_type]:
                    try:
                        ...
                        if item_type == "tasks":
                            result = runner.invoke(app, ["task", "delete", item_id, "--force"])
                        elif item_type == "issues":
                            result = runner.invoke(app, ["issue", "delete", item_id, "--force"])
                        elif item_type == "epics":
                            result = runner.invoke(app, ["epic", "delete", item_id, "--force"])
                        elif item_type == "projects":
                            result = runner.invoke(app, ["portfolio", "delete-project", item_id, "--force"])
                        elif item_type == "templates":
                            result = runner.invoke(app, ["template", "delete", item_id, "--force"])
    ...
    def test_concurrent_task_creation(self, e2e_runner, e2e_project_setup):
        ...
        try:
            ...
            with patch('ai_trackdown_pytools.core.project.Project.exists') as mock_exists, \
                ...
                mock_exists.return_value = True
                mock_tasks = [
                    Mock(id=task_id, title=f"Concurrent Task {i+1}", 
                         description="", tags=[], status="open")
        ...
        try:
            ...
            with patch('ai_trackdown_pytools.core.task.TaskManager.create_task') as mock_create:
                for i in range(1, 101):  # 100 tasks
                    ...
                    result = e2e_runner.invoke(app, [
                        "task", "create", f"Scale Test Task {i}",
                        "--priority", ["low", "medium", "high"][i % 3],
                        "--tag", f"batch-{(i-1)//10 + 1}"
                    ])
            ...
            with patch('ai_trackdown_pytools.core.project.Project.exists') as mock_exists, \
                ...
                mock_exists.return_value = True
                mock_tasks = [
                    Mock(id=f"SCALE-{i:03d}", title=f"Scale Test Task {i}", 
                         description="Scale testing", tags=[f"batch-{(i-1)//10 + 1}"], status="open")
                ...
                for limit in [10, 25, 50, 100]:
                    result = e2e_runner.invoke(app, ["search", "Scale", "--limit", str(limit)])
                    assert result.exit_code == 0
                
                # Test filtered searches
                result = e2e_runner.invoke(app, ["search", "Scale", "--status", "open", "--limit", "50"])
                assert result.exit_code == 0
