...
class TestInteractiveTaskCreation:
    ...
    def test_interactive_basic_prompts(self, mock_prompt, mock_create, mock_exists, interactive_runner):
    ...
    def test_interactive_with_confirmations(self, mock_confirm, mock_prompt, mock_create, mock_exists, interactive_runner):
        ...
        result = interactive_runner.invoke(app, [
    ...
    @patch('ai_trackdown_pytools.core.task.TaskManager.create_task')
    @patch('rich.prompt.Prompt.ask')
    def test_interactive_with_validation(self, mock_prompt, mock_create, mock_exists, interactive_runner):
        ...
        result = interactive_runner.invoke(app, ["task", "create", "--interactive", "--validate-input"])
    ...
    def test_interactive_with_different_prompt_types(self, mock_int_prompt, mock_prompt, mock_create, mock_exists, interactive_runner):
...
class TestInteractiveEpicCreation:
    ...
    @patch('ai_trackdown_pytools.core.task.TaskManager.create_task')
    @patch('rich.prompt.Prompt.ask')
    def test_interactive_epic_creation(self, mock_prompt, mock_create, mock_exists, interactive_runner):
    ...
    @patch('rich.prompt.Prompt.ask')
    @patch('rich.prompt.Confirm.ask')
    def test_interactive_epic_with_task_creation(self, mock_confirm, mock_prompt, mock_create, mock_exists, interactive_runner):
...
class TestInteractiveIssueCreation:
    ...
    @patch('ai_trackdown_pytools.core.task.TaskManager.create_task')
    @patch('rich.prompt.Prompt.ask')
    def test_interactive_bug_report(self, mock_prompt, mock_create, mock_exists, interactive_runner):
        ...
        result = interactive_runner.invoke(app, ["issue", "create", "--interactive", "--type", "bug"])
        assert result.exit_code == 0
    
    @patch('ai_trackdown_pytools.core.project.Project.exists')
    @patch('ai_trackdown_pytools.core.task.TaskManager.create_task')
    @patch('rich.prompt.Prompt.ask')
    def test_interactive_feature_request(self, mock_prompt, mock_create, mock_exists, interactive_runner):
        ...
        result = interactive_runner.invoke(app, ["issue", "create", "--interactive", "--type", "enhancement"])
        assert result.exit_code == 0
...
class TestInteractiveTemplateSelection:
    ...
    @patch('ai_trackdown_pytools.core.task.TaskManager.create_task')
    @patch('rich.prompt.Prompt.ask')
    def test_interactive_template_selection(self, mock_prompt, mock_create, mock_exists, 
                                          mock_get_template, mock_list_templates, interactive_runner):
        """Test interactive template selection from available templates."""
        mock_exists.return_value = True
        mock_list_templates.return_value = ["default", "bug_report", "feature_request", "epic", "user_story"]
        
        mock_template = {
            "name": "Bug Report Template",
            "fields": {
                "title": {"required": True},
                "severity": {"type": "choice", "choices": ["low", "medium", "high", "critical"]}
            }
        }
        ...
        result = interactive_runner.invoke(app, [
            "create", "task", "--interactive-template"
        ])
    ...
    @patch('rich.prompt.Prompt.ask')
    @patch('rich.prompt.Confirm.ask')
    def test_interactive_template_creation(self, mock_confirm, mock_prompt, mock_create_template, interactive_runner):
...
class TestInteractiveConfiguration:
    ...
    @patch('rich.prompt.Prompt.ask')
    @patch('rich.prompt.Confirm.ask')
    def test_interactive_config_setup(self, mock_confirm, mock_prompt, mock_save, mock_load, interactive_runner):
    ...
    @patch('ai_trackdown_pytools.core.config.Config.save')
    @patch('rich.prompt.Prompt.ask')
    def test_interactive_config_update(self, mock_prompt, mock_save, mock_load, interactive_runner):
...
class TestInteractiveSearch:
    ...
    @patch('rich.prompt.Prompt.ask')
    @patch('rich.prompt.Confirm.ask')
    def test_interactive_search_builder(self, mock_confirm, mock_prompt, mock_task_mgr, mock_exists, interactive_runner):
        """Test interactive search query builder."""
        mock_exists.return_value = True
        
        # Mock search results
        mock_tasks = [
            Mock(id="TSK-001", title="Test Task 1", description="First test", tags=["test"], status="open"),
            Mock(id="TSK-002", title="Bug Fix", description="Fix critical bug", tags=["bug", "critical"], status="in_progress"),
            Mock(id="EP-001", title="Test Epic", description="Epic for testing", tags=["epic"], status="active")
        ]
    ...
    @patch('ai_trackdown_pytools.core.task.TaskManager')
    @patch('rich.prompt.Prompt.ask')
    def test_interactive_search_refinement(self, mock_prompt, mock_task_mgr, mock_exists, interactive_runner):
        """Test interactive search result refinement."""
        mock_exists.return_value = True
        
        # Mock initial search results
        mock_tasks = [Mock(id=f"TSK-{i:03d}", title=f"Task {i}", description=f"Description {i}", 
                          tags=["test"], status="open") for i in range(1, 21)]
        mock_task_mgr.return_value.list_tasks.return_value = mock_tasks
        ...
        result = interactive_runner.invoke(app, [
            "search", "task", "--interactive-refine"
        ])
...
class TestInteractiveReporting:
    ...
    @patch('rich.prompt.Prompt.ask')
    @patch('rich.prompt.Confirm.ask')
    def test_interactive_status_report(self, mock_confirm, mock_prompt, mock_task_mgr, mock_exists, interactive_runner):
        ...
        mock_tasks = [
            ...
            Mock(id="TSK-002", status="in_progress", priority="medium", assignees=["bob"]),
            Mock(id="TSK-003", status="completed", priority="low", assignees=["charlie"]),
...
class TestInteractiveValidation:
    ...
    @patch('rich.prompt.Confirm.ask')
    @patch('rich.prompt.Prompt.ask')
    def test_interactive_validation_with_fixes(self, mock_prompt, mock_confirm, mock_validate, mock_exists, interactive_runner):
        ...
        result = interactive_runner.invoke(app, [
            "validate", "project", "--interactive-fix"
        ])
    ...
    @patch('rich.prompt.Confirm.ask')
    def test_interactive_task_validation(self, mock_confirm, mock_validate_task, mock_task_mgr, mock_exists, interactive_runner):
        ...
        mock_validate_task.side_effect = [
            {"valid": False, "errors": ["Missing required field: priority"], "warnings": []},
            {"valid": True, "errors": [], "warnings": ["Consider adding estimate"]},
            {"valid": False, "errors": ["Invalid status value"], "warnings": ["Old format detected"]}
        ]
        ...
        result = interactive_runner.invoke(app, [
            "validate", "tasks", "--interactive-fix"
        ])
...
class TestInteractiveHelp:
    ...
    @patch('rich.prompt.Prompt.ask')
    @patch('rich.prompt.Confirm.ask')
    def test_interactive_help_system(self, mock_confirm, mock_prompt, interactive_runner):
...
class TestRichUIElements:
    ...
    @patch('ai_trackdown_pytools.core.project.Project.exists')
    @patch('ai_trackdown_pytools.core.task.TaskManager')
    def test_table_formatting_options(self, mock_task_mgr, mock_exists, interactive_runner):
        """Test different table formatting options."""
        mock_exists.return_value = True
        
        # Mock tasks for table display
        mock_tasks = [
            Mock(id="TSK-001", title="Task 1", status="open", priority="high", assignees=["alice"]),
            Mock(id="TSK-002", title="Very Long Task Title That Should Be Truncated", 
                 status="in_progress", priority="medium", assignees=["bob", "charlie"]),
            Mock(id="TSK-003", title="Task 3", status="completed", priority="low", assignees=[])
        ]
        mock_task_mgr.return_value.list_tasks.return_value = mock_tasks
        ...
        for table_format in table_formats:
            result = interactive_runner.invoke(app, [
                "search", "task", "--table-format", table_format
            ])
