...
from ai_trackdown_pytools.utils.validation import (
    ...
    validate_ticket_file, validate_id_format, validate_relationships,
    validate_task_data, validate_epic_data, validate_issue_data,
    validate_pr_data, validate_project_data
)
from ai_trackdown_pytools.utils.frontmatter import (
    FrontmatterParser, StatusWorkflowValidator,
    parse_ticket_file, validate_ticket_data
)
...
"""
    ...
    def test_write_and_read_roundtrip(self):
        ...
        try:
            # Write file
            write_result = self.parser.write_file(temp_path, frontmatter_data, markdown_content)
            assert write_result.valid
            
            # Read file back
            read_frontmatter, read_content, read_result = self.parser.parse_file(temp_path)
...
class TestStatusWorkflowValidator:
    ...
    def test_valid_task_transitions(self):
        ...
        # Valid transitions
        result = self.workflow_validator.validate_status_transition("task", "open", "in_progress")
        assert result.valid
        
        result = self.workflow_validator.validate_status_transition("task", "in_progress", "completed")
        assert result.valid
    
    def test_invalid_task_transitions(self):
        """Test invalid task status transitions."""
        # Invalid transition
        result = self.workflow_validator.validate_status_transition("task", "completed", "open")
        assert not result.valid
