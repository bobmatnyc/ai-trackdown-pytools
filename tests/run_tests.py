...
class TestRunner:
    ...
    def run_linting(self) -> Dict[str, Any]:
        ...
        # Run ruff
        ruff_cmd = [sys.executable, "-m", "ruff", "check", str(self.src_dir), str(self.tests_dir)]
        ruff_result = subprocess.run(ruff_cmd, capture_output=True, text=True, cwd=self.project_root)
        ...
        # Run black check
        black_cmd = [sys.executable, "-m", "black", "--check", str(self.src_dir), str(self.tests_dir)]
    ...
    def generate_test_report(self, results: List[Dict[str, Any]]) -> str:
        ...
        # Save JSON report
        json_report_path = self.reports_dir / f"test-report-{datetime.now().strftime('%Y%m%d-%H%M%S')}.json"
...
"""
        ...
        # Save text report
        text_report_path = self.reports_dir / f"test-summary-{datetime.now().strftime('%Y%m%d-%H%M%S')}.txt"
...
def main():
    ...
    parser.add_argument("--fast", action="store_true", help="Run fast test suite (unit + integration)")
    parser.add_argument("--no-coverage", action="store_true", help="Skip coverage reporting")
    parser.add_argument("--verbose", "-v", action="store_true", help="Verbose output")
    parser.add_argument("--clean", action="store_true", help="Clean reports before running")
    ...
    if args.specific:
        ...
        # Run fast test suite
        results.append(runner.run_unit_tests(args.verbose, coverage=not args.no_coverage))
        results.append(runner.run_integration_tests(args.verbose))
    else:
