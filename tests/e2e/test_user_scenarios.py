...
class TestNewUserWorkflow:
    ...
    def test_complete_new_user_journey(self):
        ...
        with patch('os.getcwd', return_value=str(project_path)):
            ...
            result = self.runner.invoke(app, [
                "init", "project", 
                "--name", "My First AI Trackdown Project"
            ])
    ...
    def test_team_collaboration_workflow(self):
        ...
        with patch('os.getcwd', return_value=str(project_path)):
            ...
            result = self.runner.invoke(app, [
                "create", "epic", "User Management System",
                "--priority", "high"
            ])
            assert result.exit_code == 0
    ...
    def test_agile_development_workflow(self):
        ...
        with patch('os.getcwd', return_value=str(project_path)):
            ...
            result = self.runner.invoke(app, [
                "create", "epic", "E-commerce Product Catalog",
                "--priority", "critical"
            ])
            assert result.exit_code == 0
            ...
            technical_tasks = [
                ("Set up product database schema", "database_dev", ["backend", "database"]),
                ("Implement product search API", "backend_dev", ["backend", "api", "search"]),
                ("Create product catalog UI components", "frontend_dev", ["frontend", "ui", "components"]),
                ("Write automated tests for catalog", "qa_dev", ["testing", "automation"]),
                ("Performance optimization for search", "backend_dev", ["performance", "optimization"])
            ]
            ...
            bugs = [
                ("Product images not loading correctly", "critical", ["bug", "frontend", "images"]),
                ("Search results pagination broken", "high", ["bug", "backend", "pagination"]),
                ("Category filter showing incorrect results", "medium", ["bug", "frontend", "filtering"])
            ]
            ...
            result = self.runner.invoke(app, ["status", "tasks", "--priority", "critical"])
            assert result.exit_code == 0
            
            result = self.runner.invoke(app, ["status", "tasks", "--assignee", "frontend_dev"])
            assert result.exit_code == 0
            
            # Sprint review - check what's been completed
            result = self.runner.invoke(app, ["status", "tasks", "--status", "completed"])
            assert result.exit_code == 0
...
class TestAdvancedUserScenarios:
    ...
    def test_large_project_scenario(self):
        ...
        with patch('os.getcwd', return_value=str(project_path)):
            ...
            for i in range(50):
                ...
                result = self.runner.invoke(app, [
                    "create", "task", f"Task {i+1:03d}: Implement feature {chr(65 + i % 26)}",
                    "--priority", priority,
                    "--assignee", assignee
                ] + tag_args)
                assert result.exit_code == 0
    ...
    def test_template_customization_scenario(self):
        ...
        with patch('os.getcwd', return_value=str(project_path)):
            ...
            # Validate templates
            result = self.runner.invoke(app, ["template", "validate", "task", "default"])
            assert result.exit_code == 0
            
            # Create tasks using templates (implicit)
            result = self.runner.invoke(app, [
                "create", "task", "Template-based Task",
                "--template", "default",
                "--priority", "medium"
            ])
            assert result.exit_code == 0
...
""")
            ...
            valid_task_file = tasks_dir / [f for f in tasks_dir.glob("*.md") if "TSK-" in f.name][0]
...
class TestErrorRecoveryScenarios:
    ...
    def test_disk_space_and_resource_limits(self):
        ...
        with patch('os.getcwd', return_value=str(project_path)):
            ...
            result = self.runner.invoke(app, ["create", "task", "Large Task"], 
                                      input=f"Large task\n{large_content}\n\n\n")
            assert result.exit_code in [0, 1]
    ...
    def test_large_dataset_performance(self):
        ...
        with patch('os.getcwd', return_value=str(project_path)):
            ...
            for i in range(num_tasks):
                result = self.runner.invoke(app, [
                    "create", "task", f"Performance Task {i:03d}",
                    "--priority", "medium"
                ])
                assert result.exit_code == 0
