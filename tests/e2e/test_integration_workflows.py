...
def integration_environment():
    ...
    with patch('os.getcwd', return_value=str(project_path)):
        # Initialize project
        result = runner.invoke(app, ["init", "project", "--name", "Integration Test Project"])
...
class TestGitIntegrationWorkflows:
    ...
    def test_feature_branch_workflow(self, integration_environment):
        ...
        with patch('os.getcwd', return_value=str(project_path)):
            ...
            result = runner.invoke(app, [
                "epic", "create", "User Authentication Feature",
                "--priority", "high",
            ...
            with patch('ai_trackdown_pytools.utils.git.GitUtils.create_branch') as mock_branch:
            ...
            # Create PR
            result = runner.invoke(app, [
                "pr", "create", f"[{task_id}] Implement OAuth2 authentication",
                "--source-branch", f"feature/{task_id}-oauth2-auth",
    ...
    def test_hotfix_workflow(self, integration_environment):
        ...
        with patch('os.getcwd', return_value=str(project_path)):
            # Report critical bug
            result = runner.invoke(app, [
                "issue", "create", "Production API returning 500 errors",
                "--type", "bug",
                "--severity", "critical",
                "--component", "api",
            ...
            # Create fix task
            result = runner.invoke(app, [
                "task", "create", "Fix API 500 error in production",
                "--issue", bug_id,
                "--priority", "critical",
                "--assignee", "senior-dev",
                "--tag", "hotfix"
            ...
            with patch('ai_trackdown_pytools.utils.git.GitUtils.commit') as mock_commit:
                ...
                result = runner.invoke(app, [
                    "git", "commit",
                    "--message", f"fix: resolve API 500 error\n\nFixes #{bug_id}",
                    "--task", fix_task_id
                ])
            
            # Create PR for hotfix
            result = runner.invoke(app, [
                "pr", "create", f"[HOTFIX] Fix API 500 error",
                "--source-branch", f"hotfix/{bug_id}-api-500-error",
                "--target-branch", "main",
                "--issue", bug_id,
                "--label", "hotfix",
                "--label", "critical",
    ...
    def test_release_branch_workflow(self, integration_environment):
        ...
        with patch('os.getcwd', return_value=str(project_path)):
            ...
            with patch('ai_trackdown_pytools.utils.git.GitUtils.create_branch') as mock_branch:
                mock_branch.return_value = True
                result = runner.invoke(app, [
                    "git", "branch", "create", "release/3.0.0",
                    "--from", "develop",
                    "--epic", release_epic_id
                ])
...
class TestCrossPlatformSyncWorkflows:
    ...
    def test_github_sync_workflow(self, integration_environment):
        ...
        with patch('os.getcwd', return_value=str(project_path)):
            ...
            with patch('ai_trackdown_pytools.utils.git.GitUtils.get_remote_url') as mock_remote:
            ...
            result = runner.invoke(app, [
                "task", "create", "Implement dark mode",
                "--github-issue", "42",
    ...
    def test_jira_sync_workflow(self, integration_environment):
        ...
        with patch('os.getcwd', return_value=str(project_path)):
            ...
            # Map fields
            result = runner.invoke(app, [
                "sync", "map-fields", "jira",
                "--map", "priority:priority",
                "--map", "assignee:assignee.name",
                "--map", "status:status",
                "--map", "epic:customfield_10001"
            ])
            assert result.exit_code == 0
            ...
            result = runner.invoke(app, [
                "task", "create", "Local task for JIRA",
                "--sync-to-jira",
                "--jira-type", "Task",
    ...
    def test_webhook_automation_workflow(self, integration_environment):
        ...
        with patch('os.getcwd', return_value=str(project_path)):
            ...
            result = runner.invoke(app, [
                ...
                "--events", "task.created,task.updated,pr.opened,pr.merged"
            ])
            assert result.exit_code == 0
...
"""
    ...
    def test_template_validation_workflow(self, integration_environment):
        ...
        with patch('os.getcwd', return_value=str(project_path)):
            ...
            # Save template
            template_file = project_path / "templates" / "issue" / "feature-request.yaml"
            ...
            result = runner.invoke(app, [
                ...
                "--business_value", "This will increase user engagement significantly",
            ...
            result = runner.invoke(app, [
                ...
                "--business_value", "This feature will provide detailed insights to users",
                "--target_users", "enterprise",
                "--priority_score", "9"  # High priority requires additional fields
            ])
            assert result.exit_code == 1
...
class TestAdvancedQueryAndReporting:
    ...
    def test_complex_query_workflow(self, integration_environment):
        ...
        with patch('os.getcwd', return_value=str(project_path)):
            ...
            result = runner.invoke(app, [
                ...
                "--and", "(assignee = 'alice' OR assignee = 'bob')",
            ...
            result = runner.invoke(app, [
                "query", "related",
                "--from", "task",
                "--join", "issue ON task.issue_id = issue.id",
                "--join", "epic ON task.epic_id = epic.id",
                "--select", "task.id, task.title, issue.severity, epic.business_value",
            ...
            result = runner.invoke(app, [
                "query", "search",
                "--text", "authentication security login",
                "--in", "title,description,comments",
                "--rank-by", "relevance",
                "--boost", "title:2.0",
                "--min-score", "0.5"
            ])
    ...
    def test_custom_report_generation(self, integration_environment):
        ...
        with patch('os.getcwd', return_value=str(project_path)):
            ...
            result = runner.invoke(app, [
                ...
                "--sprints", "last-4",
                "--metrics", "completed-points,committed-points,carry-over",
                "--group-by", "team",
                "--format", "json",
    ...
    def _create_query_test_data(self, runner):
        ...
        for i in range(30):
            ...
            runner.invoke(app, [
                "task", "create", f"Task {i+1}: Feature implementation",
                "--epic", epic_id,
                "--assignee", assignee,
                "--priority", priority,
                "--status", status,
        ...
        for i in range(10):
            ...
            runner.invoke(app, [
                "issue", "create", f"Issue {i+1}",
                "--type", issue_type,
                "--severity", severity,
                "--component", ["frontend", "backend", "api", "database"][i % 4]
            ])
        ...
        with patch('os.getcwd', return_value=str(project_path)):
            ...
            roles = [
                ...
                },
                {
                    "name": "developer",
                    "permissions": [
                        "task:create", "task:update:assigned", "task:read",
                        "issue:create", "issue:comment", "pr:create"
                    ],
            ...
            with patch.dict(os.environ, {"AITRACKDOWN_USER": "bob"}):
                ...
                # Cannot manage users
                result = runner.invoke(app, ["auth", "user", "create", "--username", "test"])
                assert result.exit_code == 1
    ...
    def test_data_encryption(self, integration_environment):
        ...
        with patch('os.getcwd', return_value=str(project_path)):
            ...
            result = runner.invoke(app, [
                ...
                "--fields", "api_keys,tokens,passwords",
            ...
            result = runner.invoke(app, [
                "task", "create", "Deploy API with credentials",
                "--metadata", "api_key:sk-1234567890abcdef",
                "--metadata", "db_password:super_secret_pass",
                "--tag", "deployment"
            ])
            assert result.exit_code == 0
            task_id = self._extract_id(result.stdout, "TSK")
