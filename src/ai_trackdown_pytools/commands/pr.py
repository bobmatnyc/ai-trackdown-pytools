...
def list(
    ...
    author_filter: Optional[str] = typer.Option(None, "--author", "-a", help="Filter by author"),
    reviewer_filter: Optional[str] = typer.Option(None, "--reviewer", "-r", help="Filter by reviewer"),
    target_filter: Optional[str] = typer.Option(None, "--target", "-t", help="Filter by target branch"),
    show_closed: bool = typer.Option(False, "--closed", "-c", help="Include closed PRs"),
    show_draft: bool = typer.Option(True, "--draft", "-d", help="Include draft PRs"),
    limit: int = typer.Option(20, "--limit", "-l", help="Maximum number of PRs to show"),
) -> None:
    """List pull requests."""
    project_path = Path.cwd()
...
def show(
    ...
    show_commits: bool = typer.Option(False, "--commits", "-c", help="Show commit history"),
) -> None:
...
[dim]Updated:[/dim] {pr.updated_at.strftime('%Y-%m-%d %H:%M')}"""
    ...
    if show_commits and pr.metadata.get("commits"):
        ...
        for commit in pr.metadata.get("commits", []):
            table.add_row(
                commit.get("sha", "")[:8],
                commit.get("message", "")[:60] + "..." if len(commit.get("message", "")) > 60 else commit.get("message", ""),
                commit.get("author", "")
            )
...
def update(
    ...
    description: Optional[str] = typer.Option(None, "--description", "-d", help="Update PR description"),
    status: Optional[str] = typer.Option(None, "--status", "-s", help="Update PR status (draft, open, ready, merged, closed)"),
    add_reviewer: Optional[List[str]] = typer.Option(None, "--add-reviewer", help="Add reviewers"),
    remove_reviewer: Optional[List[str]] = typer.Option(None, "--remove-reviewer", help="Remove reviewers"),
...
def review(
    ...
    action: str = typer.Option(..., "--action", "-a", help="Review action (approve, request_changes, comment)"),
    message: Optional[str] = typer.Option(None, "--message", "-m", help="Review message"),
    ...
    if action == "approve":
        ...
        console.print(f"[yellow]Changes requested for PR {pr_id} by {reviewer}[/yellow]")
    else:
...
def merge(
    ...
    merge_type: str = typer.Option("merge", "--type", "-t", help="Merge type (merge, squash, rebase)"),
    delete_branch: bool = typer.Option(False, "--delete-branch", "-d", help="Delete source branch after merge"),
) -> None:
