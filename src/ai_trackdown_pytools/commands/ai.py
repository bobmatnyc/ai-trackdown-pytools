...
def track_tokens(
    session_id: Optional[str] = typer.Argument(None, help="Session ID to track tokens for"),
    input_tokens: Optional[int] = typer.Option(None, "--input", "-i", help="Number of input tokens"),
    output_tokens: Optional[int] = typer.Option(None, "--output", "-o", help="Number of output tokens"),
    ...
    task_id: Optional[str] = typer.Option(None, "--task", "-t", help="Associate with task ID"),
    ...
    if token_file.exists():
        ...
        token_data = {"sessions": [], "total_stats": {"input_tokens": 0, "output_tokens": 0, "total_cost": 0.0}}
...
def token_stats(
    task_id: Optional[str] = typer.Option(None, "--task", "-t", help="Show stats for specific task"),
    model_filter: Optional[str] = typer.Option(None, "--model", "-m", help="Filter by model"),
    detailed: bool = typer.Option(False, "--detailed", "-d", help="Show detailed session breakdown"),
...
[dim]Average Cost per Session:[/dim] ${total_cost/session_count:.4f}""",
    ...
    if export:
        ...
        with open(export_path, 'w', newline='') as csvfile:
            fieldnames = ['session_id', 'timestamp', 'model', 'input_tokens', 'output_tokens', 'total_tokens', 'cost', 'task_id', 'description']
...
def generate_llms_txt(
    output: Optional[str] = typer.Option("llms.txt", "--output", "-o", help="Output filename"),
    include_code: bool = typer.Option(True, "--include-code", help="Include code files"),
    include_docs: bool = typer.Option(True, "--include-docs", help="Include documentation"),
    include_tests: bool = typer.Option(False, "--include-tests", help="Include test files"),
    max_file_size: int = typer.Option(50000, "--max-size", help="Maximum file size in bytes"),
    exclude_patterns: Optional[List[str]] = typer.Option(None, "--exclude", help="Patterns to exclude"),
) -> None:
    ...
    code_extensions = {'.py', '.js', '.ts', '.jsx', '.tsx', '.java', '.cpp', '.c', '.h', '.rs', '.go', '.rb', '.php'}
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        ...
        def write_tree(path: Path, prefix: str = "", max_depth: int = 3, current_depth: int = 0):
...
@app.command("context")
def context(
    task_id: Optional[str] = typer.Argument(None, help="Task ID to generate context for"),
    output: Optional[str] = typer.Option(None, "--output", "-o", help="Output file path"),
    include_related: bool = typer.Option(True, "--include-related", help="Include related tasks"),
    include_files: bool = typer.Option(True, "--include-files", help="Include relevant files"),
    format: str = typer.Option("markdown", "--format", "-f", help="Output format (markdown, json, txt)"),
) -> None:
            ...
            if task_id:
                ...
                if "related_tasks" in context_data:
                    ...
                    for related in context_data["related_tasks"]:
                        ...
                        f.write(f"- **{related['type'].title()}:** {rel_task.id} - {rel_task.title}\n")
            else:
