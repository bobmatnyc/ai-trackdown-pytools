...
def list(
    ...
    show_completed: bool = typer.Option(False, "--completed", "-c", help="Include completed tasks"),
    ...
    if detailed and subtasks:
        ...
        for subtask_id in subtasks:
            ...
            if subtask:
                table.add_row(
                    subtask.id,
                    subtask.title[:50] + "..." if len(subtask.title) > 50 else subtask.title,
                    subtask.status,
                    subtask.priority
                )
...
def update(
    ...
    description: Optional[str] = typer.Option(None, "--description", "-d", help="Update task description"),
    status: Optional[str] = typer.Option(None, "--status", "-s", help="Update task status"),
    priority: Optional[str] = typer.Option(None, "--priority", "-p", help="Update task priority"),
    assignee: Optional[List[str]] = typer.Option(None, "--assignee", "-a", help="Update task assignees"),
    estimate: Optional[str] = typer.Option(None, "--estimate", "-e", help="Update time estimate"),
    tag: Optional[List[str]] = typer.Option(None, "--tag", "-t", help="Update task tags"),
    add_tag: Optional[List[str]] = typer.Option(None, "--add-tag", help="Add tags to task"),
    remove_tag: Optional[List[str]] = typer.Option(None, "--remove-tag", help="Remove tags from task"),
) -> None:
...
def block(
    ...
    reason: Optional[str] = typer.Option(None, "--reason", "-r", help="Reason for blocking"),
) -> None:
...
def unblock(
    ...
    blocked_by: Optional[str] = typer.Argument(None, help="Specific blocking task to remove"),
) -> None:
    """Unblock a task."""
    project_path = Path.cwd()
    ...
    if blocked_by:
        ...
        if blocked_by in blocked_by_list:
            ...
            console.print(f"[yellow]Task {task_id} is not blocked by {blocked_by}[/yellow]")
            return
    else:
...
def start(
    ...
    comment: Optional[str] = typer.Option(None, "--comment", "-c", help="Start comment"),
) -> None:
