...
@click.option('--ticket-type', type=click.Choice(['task', 'epic', 'issue', 'pr', 'project']),
...
def file(file_path: Path, ticket_type: Optional[str], output_format: str, verbose: bool):
...
def directory(directory: Path, pattern: str, ticket_type: Optional[str], 
             output_format: str, summary_only: bool):
...
@click.argument('ticket_type', type=click.Choice(['task', 'epic', 'issue', 'pr', 'project']))
@click.option('--input-format', type=click.Choice(['json', 'yaml']), default='json',
