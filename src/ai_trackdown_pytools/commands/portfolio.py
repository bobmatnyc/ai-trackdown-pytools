...
def overview(
    ...
    show_completed: bool = typer.Option(False, "--completed", "-c", help="Include completed items"),
) -> None:
    ...
    regular_tasks = [t for t in all_tasks if not any(tag in t.tags for tag in ["epic", "issue", "pull-request"])]
    ...
    for epic in epics:
        ...
        if subtasks:
            completed = sum(1 for st_id in subtasks 
                          if task_manager.load_task(st_id) and task_manager.load_task(st_id).status == "completed")
...
def backlog(
    ...
    epic_filter: Optional[str] = typer.Option(None, "--epic", "-e", help="Filter by epic"),
    ...
    open_tasks = [t for t in all_tasks if t.status in ["open", "in_progress", "blocked"]]
    ...
    open_tasks.sort(key=lambda t: (priority_order.get(t.priority, 0), t.created_at), reverse=True)
    ...
    if grouped:
        ...
        for priority in ["critical", "high", "medium", "low"]:
            if priority in priority_groups:
                ...
                for task in tasks:
                    ...
                    epic = task.metadata.get("epic", "")[:10] if task.metadata.get("epic") else ""
        ...
        for task in open_tasks:
            task_type = _get_task_type(task)
            assignee = ", ".join(task.assignees[:2]) if task.assignees else "Unassigned"
            epic = task.metadata.get("epic", "")[:8] if task.metadata.get("epic") else ""
...
def roadmap(
    ...
    timeline: Optional[str] = typer.Option(None, "--timeline", "-t", help="Timeline filter (quarter, month)"),
    ...
    priority_order = {"critical": 4, "high": 3, "medium": 2, "low": 1}
    epics.sort(key=lambda t: (priority_order.get(t.priority, 0), t.created_at), reverse=True)
    ...
    if format == "tree":
        ...
        for epic in epics:
            ...
            if not epic_only and subtasks:
                ...
                for subtask_id in subtasks[:10]:  # Limit to first 10 subtasks
                    ...
                    if subtask:
                        status_icon = "✅" if subtask.status == "completed" else "🔄" if subtask.status == "in_progress" else "⭕"
        ...
        for epic in epics:
            ...
            table.add_row(
                epic.title[:30] + "..." if len(epic.title) > 30 else epic.title,
                epic.metadata.get("goal", "")[:40] + "..." if len(epic.metadata.get("goal", "")) > 40 else epic.metadata.get("goal", ""),
                epic.status,
                epic.priority,
                f"{progress_bar} {progress}%",
                str(len(subtasks))
...
def sprint(
    ...
    capacity: Optional[int] = typer.Option(None, "--capacity", "-c", help="Sprint capacity in story points"),
) -> None:
    ...
    if action == "create":
        ...
        existing = next((s for s in sprints_data["sprints"] if s["name"] == sprint_name), None)
        if existing:
...
[dim]Capacity:[/dim] {capacity or 'Not set'}""",
        ...
        sprint = next((s for s in sprints_data["sprints"] if s["name"] == current_sprint_name), None)
        if not sprint:
...
[dim]Tasks:[/dim] {len(sprint_tasks)} total, {len(completed_tasks)} completed""",
        ...
        console.print(f"[green]Sprint '{current_sprint_name}' closed successfully[/green]")
