...
[dim]Created:[/dim] {project.created_at.strftime('%Y-%m-%d %H:%M:%S')}""",
    ...
    regular_tasks = [t for t in tasks if not any(tag in t.tags for tag in ["epic", "issue", "pull-request"])]
...
[dim]Commit:[/dim] {git_status.get('commit', 'unknown')[:8]}
    ...
    if verbose:
        ...
        if recent_tasks:
            ...
            for task in recent_tasks:
                ...
                task_tree.add(f"[{status_color}]{task.title}[/{status_color}] ({task.status})")
