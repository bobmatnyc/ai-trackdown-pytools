...
class SchemaValidator:
    ...
    def validate_ticket(self, ticket_data: Dict[str, Any], ticket_type: str) -> ValidationResult:
    ...
    def _validate_against_json_schema(self, data: Dict[str, Any], schema_name: str) -> ValidationResult:
        ...
        try:
            ...
            if e.validator and e.validator_value:
                result.add_error(f"Validation rule '{e.validator}' failed with value: {e.validator_value}")
    ...
    def _validate_against_pydantic_model(self, data: Dict[str, Any], ticket_type: str) -> ValidationResult:
    ...
    def _validate_custom_rules(self, data: Dict[str, Any], ticket_type: str) -> ValidationResult:
        ...
        if 'id' in data:
            ...
            if not re.match(expected_pattern, data['id']):
                result.add_error(f"ID '{data['id']}' doesn't match expected pattern for {ticket_type}")
    ...
    def _validate_task_specific_rules(self, data: Dict[str, Any], result: ValidationResult) -> None:
    ...
    def _validate_epic_specific_rules(self, data: Dict[str, Any], result: ValidationResult) -> None:
        """Validate epic-specific rules."""
        # Business value should be provided for high priority epics
        if data.get('priority') in ['high', 'critical'] and not data.get('business_value', '').strip():
    ...
    def _validate_issue_specific_rules(self, data: Dict[str, Any], result: ValidationResult) -> None:
        """Validate issue-specific rules."""
        # Bug reports should have reproduction steps
        if data.get('issue_type') == 'bug':
            required_fields = ['steps_to_reproduce', 'expected_behavior', 'actual_behavior']
            for field in required_fields:
                if not data.get(field, '').strip():
                    result.add_warning(f"Bug reports should have '{field}' defined")
        
        # Feature requests should have clear description
        if data.get('issue_type') == 'feature' and not data.get('description', '').strip():
            result.add_warning("Feature requests should have detailed description")
    
    def _validate_pr_specific_rules(self, data: Dict[str, Any], result: ValidationResult) -> None:
        """Validate pull request-specific rules."""
        # Breaking changes should be clearly marked
        if data.get('breaking_changes', False) and data.get('priority') not in ['high', 'critical']:
            result.add_warning("PRs with breaking changes should typically have high or critical priority")
        ...
        if total_changes > 500 and not data.get('reviewers'):
            result.add_warning("Large PRs (>500 lines changed) should have reviewers assigned")
    
    def _validate_project_specific_rules(self, data: Dict[str, Any], result: ValidationResult) -> None:
    ...
    def _validate_relationships(self, data: Dict[str, Any], result: ValidationResult) -> None:
        ...
        reference_fields = ['dependencies', 'child_issues', 'child_tasks', 'related_issues', 'closes_issues', 'epics']
        for field in reference_fields:
            if field in data and data[field]:
                self._validate_reference_ids(data[field], result, field)
    
    def _check_circular_dependencies(self, ticket_id: str, dependencies: List[str], result: ValidationResult) -> None:
        """Check for circular dependencies (basic check)."""
        if ticket_id in dependencies:
            result.add_error(f"Circular dependency detected: {ticket_id} cannot depend on itself")
    
    def _validate_reference_ids(self, ids: List[str], result: ValidationResult, field_name: str) -> None:
        ...
        for ref_id in ids:
            if not any(re.match(pattern, ref_id) for pattern in valid_patterns):
                result.add_error(f"Invalid reference ID format in {field_name}: {ref_id}")
    ...
    def _validate_model_instance(self, instance, result: ValidationResult) -> None:
        ...
        if hasattr(instance, 'actual_hours') and hasattr(instance, 'estimated_hours'):
            if (instance.actual_hours and instance.estimated_hours and 
                instance.actual_hours > instance.estimated_hours * 3):
    ...
    def _preprocess_datetime_fields(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        date_fields = ['due_date', 'target_date', 'start_date', 'end_date', 'target_completion']
            ...
            if not isinstance(variables, dict):
                ...
                for var_name, var_config in variables.items():
                    if not isinstance(var_config, dict):
                        errors.append(f"Variable '{var_name}' configuration must be a dictionary")
...
def validate_ticket_file(file_path: Path, ticket_type: Optional[str] = None) -> ValidationResult:
    ...
    try:
        frontmatter_data, content, parse_result = parse_ticket_file(file_path, validate=True)
        ...
        if frontmatter_data.get('id'):
            ...
            if file_path.name.lower() != expected_filename:
                result.add_warning(f"File name '{file_path.name}' doesn't match ticket ID '{frontmatter_data['id']}'")
...
def validate_id_format(ticket_id: str, ticket_type: str) -> ValidationResult:
    ...
    if not re.match(expected_pattern, ticket_id):
        result.add_error(f"ID '{ticket_id}' doesn't match expected pattern {expected_pattern} for {ticket_type}")
    ...
    for ticket in tickets:
        ...
        for dep_id in dependencies:
            if dep_id not in ticket_ids:
                result.add_warning(f"Ticket {ticket_id} depends on non-existent ticket {dep_id}")
        
        # Check parent exists
        parent_id = ticket.get('parent')
        if parent_id and parent_id not in ticket_ids:
            result.add_warning(f"Ticket {ticket_id} has non-existent parent {parent_id}")
        ...
        for field in child_fields:
            ...
            for child_id in children:
                if child_id not in ticket_ids:
                    result.add_warning(f"Ticket {ticket_id} references non-existent child {child_id}")
...
def _check_circular_dependencies_deep(tickets: List[Dict[str, Any]], result: ValidationResult) -> None:
    ...
    for ticket_id in dependencies:
        if ticket_id not in visited:
            if has_cycle(ticket_id):
                result.add_error(f"Circular dependency detected involving ticket {ticket_id}")
