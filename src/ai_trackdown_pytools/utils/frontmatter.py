...
class FrontmatterParser:
    ...
    def parse_file(self, file_path: Path) -> Tuple[Dict[str, Any], str, ValidationResult]:
    ...
    def parse_string(self, content: str) -> Tuple[Dict[str, Any], str, ValidationResult]:
        ...
        try:
            ...
            if not isinstance(parsed_data, dict):
                validation_result.add_error("Frontmatter must be a YAML object/dictionary")
        ...
        if self.validate_schema and self.validator:
            ...
            if ticket_type:
                schema_validation = self.validator.validate_ticket(parsed_data, ticket_type)
                validation_result.merge(schema_validation)
            else:
                validation_result.add_warning("Could not detect ticket type for schema validation")
        
        return parsed_data, markdown_content, validation_result
    
    def write_file(self, file_path: Path, frontmatter_data: Dict[str, Any], 
                   content: str, validate: bool = True) -> ValidationResult:
        ...
        if validate and self.validate_schema and self.validator:
            ...
            if ticket_type:
                schema_validation = self.validator.validate_ticket(frontmatter_data, ticket_type)
                validation_result.merge(schema_validation)
    ...
    def update_frontmatter(self, file_path: Path, updates: Dict[str, Any], 
    ...
    def validate_frontmatter(self, frontmatter_data: Dict[str, Any], 
                           ticket_type: Optional[str] = None) -> ValidationResult:
...
class StatusWorkflowValidator:
    ...
    def validate_status_transition(self, ticket_type: str, from_status: str, 
                                  to_status: str) -> ValidationResult:
        ...
        if from_status not in workflow:
            result.add_error(f"Invalid current status '{from_status}' for {ticket_type}")
            return result
...
def parse_ticket_file(file_path: Path, validate: bool = True) -> Tuple[Dict[str, Any], str, ValidationResult]:
...
def validate_ticket_data(data: Dict[str, Any], ticket_type: Optional[str] = None) -> ValidationResult:
...
def write_ticket_file(file_path: Path, frontmatter_data: Dict[str, Any], 
                     content: str = "", validate: bool = True) -> ValidationResult:
